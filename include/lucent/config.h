...
// `PSXPORT_DEBUG`) renames them. THE ONLY ORDER-FREE WAY TO DO THAT IS AT BUILD TIME, so that is
// the primary mechanism — define these when compiling lucent, or set the identically-named CMake
...
// reconfiguration; they re-read the environment rather than being ignored, but they cannot recover
// a line that was already dropped.
#pragma once
...
// lookup and re-arms the logger's channel set, so a call after something has already read config
// (or already logged) still takes effect rather than being silently half-applied. It cannot un-drop
// a debug line that was emitted while the wrong prefix was in force — set it early, or better,
// build with LUCENT_CHANNEL_ENV / LUCENT_LOG_FILE_ENV and never call it at all.
...
// An integer-valued setting: a frame count, a scale, a port. Returns `fallback` when the variable
// is absent or does not parse. Accepts decimal and 0x-prefixed hex.
...
// True when the variable is present at all, whatever its value. Use this to distinguish "unset"
// from "set to something falsy", which `flag` deliberately collapses.
...
// Forget every cached lookup. Exists for tests, which need to set an environment variable and see
// it take effect; production code should not need this.
void reset_cache();
