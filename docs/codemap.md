...
| Bounded HTTP | real, tested | `include/lucent/http.h`, `src/http.cpp` | Loopback is the default. An explicit local-network scope binds all IPv4 interfaces for authenticated, opt-in sharing routes; Lucent supplies no authentication or TLS. One request per connection; no keep-alive or chunked transfer. Request framing and diagnostics use the raw logging boundary and do not impose the optional formatting API on consumers. |
