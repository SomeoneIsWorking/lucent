...
| Browser static-host isolation | Same-origin service-worker registration, bounded first-navigation reload and explicit application-resource offline cache; consumer supplies its release manifest | `platforms/web/isolation.mjs`, `platforms/web/service-worker.js` | `prepareApplication` |
| Browser private storage | Optional Emscripten owner | `include/lucent/web.h`, `src/web.cpp`, `platforms/web/storage.mjs` | Mount and unmount OPFS on the application worker and stage local files as bounded streams with progress. Native consumers retain parsing, complete-install validation and accepted-install publication. Persistence grants are reported to the consumer rather than assumed. |
...
| Immutable native backing files | Optional GIO worker owner | `include/lucent/file_store.h`, `src/file_store.cpp`, `src/file_store_directory.h`, `src/file_store_directory.cpp`, `tests/test_file_store.cpp` | `lucent::file_store::FileStore` owns asynchronous byte staging, aggregate reservations, and result lifetime. Its private directory peer owns cryptographic names, exclusive creation, permissions, writes, and exact-file cleanup. Consumers choose bytes/suffix/budgets and release native readers before terminal store teardown. |
...
| Android import foreground lifetime | Java lifecycle tested; Android API compiled by consumers | `platforms/android/java/io/github/someoneisworking/lucent/{LucentImportLifetime,LucentImportNotification,LucentImportProgress,LucentImportService}.java` | One main-thread import lifetime starts foreground protection once, updates notifications without service starts, and stops through `Context.stopService`. The shared progress presentation supplies a persistent indeterminate bar; titles supply wording/channel and register the non-exported dataSync service; import execution and validation remain separate. |
| Platform user data and Android imports | real, partly platform-compiled | `include/lucent/platform.h`, `include/lucent/platform_c.h`, `src/platform.cpp`, `platforms/android/java/io/github/someoneisworking/lucent/LucentDocumentImport.java` | Android shells provide their app-private root. Lucent owns saved pending picker identity and new-Activity callback rebinding through `LucentImportRequest`, persisted SAF read grants, bounded private staging, safe rejection discard, source-document disposal after title validation, and recovery-safe promotion of a title-validated staging directory or contained child via `LucentImportPromotion`, and a throttled main-thread account of a copy in flight (entries, bytes, current name; no total, because a SAF tree is enumerated as it is walked); a title owns selection wording and identity/completeness validation. |
| ZIP install extraction | real, tested | `include/lucent/zip.h`, `src/zip.cpp`, `src/zip_reader.{h,cpp}`, `src/zip_directory.{h,cpp}`, `src/zip_entry.{h,cpp}`, `tests/test_zip*.cpp` | Optional `lucent::zip` target uses zlib and is disabled by default for embedded consumers. The reader owns bounded random-access file/span I/O; the directory owner validates metadata, names and aggregate budgets; the entry owner streams stored/deflate bytes and CRC; `zip.cpp` owns selection and failure-atomic publication. Filename selection and whole-archive extraction never map or buffer complete files or entries. The content-matcher span API retains its explicit per-entry buffer bound and one nested ZIP. |
...
- Supply immutable local paths to native readers of in-memory content: `lucent::file_store::FileStore`
  in `include/lucent/file_store.h`; it publishes only completely written files and deletes owned paths
  at terminal teardown, without depending on a subsequent GLib iteration.
