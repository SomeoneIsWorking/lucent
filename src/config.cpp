...
// The names lucent reads for its own two settings. Build-time, because that is the only way for them
// to be right before anything has had a chance to call a setter — see the long note in config.h.
// Empty means "use the default, `<prefix>LUCENT_DEBUG` / `<prefix>LUCENT_LOG_FILE`".
...
const std::string& cached(std::string_view name) { return cached_full(full_name(name)); }

// The variable lucent reads for its own channel list / log file. A name configured by the consumer
// is used verbatim: it must NOT pick up the prefix, or it would inherit set_prefix's ordering, which
// is the very dependency naming it was supposed to remove.
...
// tell the logger to re-arm. The logger's own mutex is held while it calls back into config (it reads
// the channel list under its lock), so taking the two in the other order here would be a textbook
// lock-order inversion. Nothing between the two statements can observe an inconsistent state: the
// logger re-reads the name when it re-loads.
void set_prefix(std::string_view prefix) {
  {
    State& st = state();
    std::lock_guard lock(st.mutex);
    if (st.prefix == prefix) return;
...
void set_channel_env(std::string_view env_name) {
  {
    State& st = state();
    std::lock_guard lock(st.mutex);
    if (st.channel_env == env_name) return;
